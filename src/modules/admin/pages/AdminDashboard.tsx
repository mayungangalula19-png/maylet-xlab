...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // ============================================================
      // FETCH ALL STATS FROM DATABASE (all queries run in parallel)
      // ============================================================
      ...
      const [
        ...
        supabase.from('payments').select('amount').gte('created_at', thirtyDaysAgo.toISOString()),
      ...
      // ============================================================
      // FETCH RECENT USERS / PROJECTS / ACTIVITIES (in parallel)
      // ============================================================
      ...
      if (usersData) {
        // One grouped query for project counts instead of one query per user (N+1 fix)
      ...
      if (projectsData) {
        // One batched profile lookup instead of one query per project (N+1 fix)
  ...
  useEffect(() => {
    ...
    // Debounce realtime-triggered refetches: events across 4 tables collapse
    // into one refetch (the full stats query runs ~15 queries each time).
    let refetchTimer: ReturnType<typeof setTimeout> | undefined;
    const debouncedRefetch = () => {
      clearTimeout(refetchTimer);
      refetchTimer = setTimeout(fetchDashboardData, 800);
    };
